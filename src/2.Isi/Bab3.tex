...
Berikut merupakan contoh \textit{request} untuk API \textit{Get All Payroll Item}. Pada gambar \ref{fig:request_get_all_payroll_item} menunjukkan contoh \textit{request} yang dikirimkan ke \textit{endpoint} \texttt{/payroll-items} menggunakan metode HTTP \textit{GET} dengan menyertakan \textit{query parameters} untuk \textit{pagination} seperti \textit{page}, \textit{row}, \textit{sort}, dan \textit{search}.
\begin{figure}
    \centering
...
Gambar \ref{fig:request_get_payroll_item_by_id} merupakan contoh \textit{request} untuk API \textit{Get Payroll Item by ID}. Pada gambar \ref{fig:request_get_payroll_item_by_id} menunjukkan contoh \textit{request} yang dikirimkan ke \textit{endpoint} \texttt{/payroll-items/:id} menggunakan metode HTTP \textit{GET} dengan menyertakan \textit{path parameters}, \texttt{payroll\_item\_id}, untuk menentukan komponen gaji yang ingin diambil detailnya.
...
Gambar \ref{fig:response_get_payroll_item_by_id} merupakan contoh \textit{response} untuk API \textit{Get Payroll Item by ID}. Pada gambar \ref{fig:response_get_payroll_item_by_id} menunjukkan contoh \textit{response} yang diterima dari \textit{server}, berisi informasi rinci mengenai komponen gaji tertentu, termasuk nama komponen gaji, jenis komponen gaji, besaran komponen gaji, serta status komponen gaji (aktif atau tidak aktif).
\begin{figure}
    \centering
...
Gambar \ref{fig:request_create_payroll_item} merupakan contoh \textit{request} dan \textit{response} untuk API \textit{Create Payroll Item}. Pada gambar \ref{fig:request_create_payroll_item} menunjukkan contoh \textit{request} yang dikirimkan ke \textit{endpoint} \texttt{/payroll-items} menggunakan metode HTTP \textit{POST} dengan menyertakan \textit{request body} yang berisi data komponen gaji baru yang akan dibuat.
...
Gambar \ref{fig:response_update_payroll_item} merupakan contoh \textit{response} untuk API \textit{Update Payroll Item}. Pada gambar \ref{fig:response_update_payroll_item} menunjukkan contoh \textit{response} yang diterima dari \textit{server}, berisi informasi mengenai komponen gaji yang telah diperbarui, termasuk nama komponen gaji, jenis komponen gaji, besaran komponen gaji, serta status komponen gaji (aktif atau tidak aktif).
\begin{figure}
    \centering
...
Gambar \ref{fig:response_delete_payroll_item} merupakan contoh \textit{response} untuk API \textit{Delete Payroll Item}. Pada gambar \ref{fig:response_delete_payroll_item} menunjukkan contoh \textit{response} yang diterima dari \textit{server}, berisi informasi mengenai keberhasilan penghapusan komponen gaji dengan menampilkan \textit{payroll item ID} dari komponen gaji yang telah dihapus.
\begin{figure}
    \centering
