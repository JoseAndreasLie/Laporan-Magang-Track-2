...
Gambar \ref{fig:flowchart_get_on_boarding} merupakan \textit{flowchart} API \textit{Get On Boarding}. \textit{Flowchart} dimulai dari pengecekan autentikasi pengguna. Jika autentikasi gagal, sistem akan mengembalikan \textit{error} 401. Jika autentikasi berhasil, sistem akan memeriksa \textit{permission data setup} untuk mengakses data impor. Jika pengguna tidak ada \textit{permission on boarding}, sistem akan mengembalikan \textit{error} 403. Setelah itu, sistem mengambil data semua riwayat impor data klien baru. Setelah data berhasil diambil, sistem mengembalikan respons (\textit{response}) kepada pengguna yang berisi daftar riwayat impor data.
\begin{figure}[H]
    \centering
    \fbox{\includegraphics[height=0.6\textheight]{assets/pics/fig_flowchart_get_on_boarding.jpg}}
    \caption{\textit{Flowchart} alur sistem API \textit{Get On Boarding}}
    \label{fig:flowchart_get_on_boarding}
\end{figure}

Pada gambar \ref{fig:request_get_on_boarding} merupakan contoh \textit{request} untuk API \textit{Get On Boarding} yang dikirimkan ke \textit{endpoint} \texttt{/on-boarding} menggunakan metode HTTP \textit{GET}.
\begin{figure}[H]
    \centering
...
Pada gambar \ref{fig:response_get_on_boarding} merupakan contoh \textit{response} untuk API \textit{Get On Boarding} yang diterima dari \textit{server}, berisi daftar riwayat impor data klien baru, termasuk informasi seperti \textit{ID}, nama berkas (\textit{file name}), dan modul yang diimpor.
\begin{figure}[H]
    \centering
...
\textit{On Boarding Import} adalah \textit{endpoint} API yang berfungsi untuk mengimpor data klien baru dari berkas CSV yang diunggah oleh pengguna. API ini digunakan di sisi \textit{frontend} untuk memproses impor data klien baru ke dalam sistem berdasarkan data yang terdapat pada berkas CSV.
Gambar \ref{fig:flowchart_on_boarding_import} dan Gambar \ref{fig:flowchart_import_data_for_all_module} merupakan \textit{flowchart} API \textit{On Boarding Import}. \textit{Flowchart} dimulai dari pengecekan autentikasi pengguna. Jika autentikasi gagal, sistem akan mengembalikan \textit{error} 401. Jika autentikasi berhasil, sistem akan memeriksa \textit{permission data setup} untuk mengimpor data klien baru. Jika pengguna tidak ada \textit{permission on boarding}, sistem akan mengembalikan \textit{error} 403. Setelah itu, sistem memproses data dari \textit{request body} untuk mengimpor data yang sudah diunggah ke dalam table modul masing-masing. Setelah data berhasil diimpor, sistem mengembalikan respons (\textit{response}) kepada pengguna yang berisi informasi mengenai hasil impor data.
\begin{figure}[H]
    \centering
...
Pada gambar \ref{fig:flowchart_import_data_for_all_module} merupakan subproses dari \textit{flowchart} API \textit{On Boarding Import} yang bernama \textit{Import Data for All Module} yang menjelaskan proses untuk memastikan tidak ada data yang duplikat dan memproses memasukkan data dari masing-masing modul.
\begin{figure}[H]
    \centering
...
Pada gambar \ref{fig:request_on_boarding_import} menunjukkan contoh \textit{request} untuk API \textit{On Boarding Import} yang dikirimkan ke \textit{endpoint} \texttt{/on-boarding/import-csv} menggunakan metode HTTP \textit{POST} dengan menyertakan \textit{request body} yang berisi data dari berkas CSV yang diunggah oleh pengguna.
\begin{figure}[H]
    \centering
...
Pada gambar \ref{fig:response_on_boarding_import} menunjukkan contoh \textit{response} untuk API \textit{On Boarding Import} yang diterima dari \textit{server}, berisi informasi mengenai hasil impor data klien baru, termasuk jumlah data yang berhasil diimpor dan jumlah data yang gagal diimpor beserta alasan kegagalannya.
...
\textit{Get Overtime List} adalah \textit{endpoint} API yang berfungsi untuk mengambil semua permintaan lembur (\textit{Overtime}) yang ada di dalam sistem. API ini digunakan di sisi \textit{frontend} untuk menampilkan daftar permintaan lembur dalam page \textit{Overtime}.
Gambar \ref{fig:flowchart_get_overtime_list} merupakan \textit{flowchart} API \textit{Get Overtime List}. \textit{Flowchart} dimulai dari pengecekan autentikasi pengguna. Jika autentikasi gagal, sistem akan mengembalikan \textit{error} 401. Jika autentikasi berhasil, sistem akan memeriksa \textit{permission overtime} untuk mengakses data permintaan lembur. Jika pengguna tidak ada \textit{permission overtime}, sistem akan mengembalikan \textit{error} 403. Setelah itu, sistem mengambil data semua permintaan lembur. Setelah data berhasil diambil, sistem mengembalikan respons (\textit{response}) kepada pengguna yang berisi daftar permintaan lembur.
\begin{figure}[H]
    \centering
...
Pada gambar \ref{fig:request_get_overtime_list} menunjukkan contoh \textit{request} untuk API \textit{Get Overtime List} yang dikirimkan ke \textit{endpoint} \texttt{/overtime} menggunakan metode HTTP \textit{GET} dengan menyertakan beberapa \textit{query parameters} untuk kebutuhan \textit{pagination}, seperti \texttt{page} dan \texttt{row}.
\begin{figure}[H]
    \centering
...
Pada gambar \ref{fig:response_get_overtime_list} menunjukkan contoh \textit{response} untuk API \textit{Get Overtime List} yang diterima dari \textit{server}, berisi daftar permintaan lembur, termasuk informasi seperti \textit{overtime ID}, nama pengguna, tanggal lembur, durasi, dan status persetujuan.

\begin{figure}[H]
    \centering
    \fbox{\includegraphics[height=0.7\textheight]{assets/pics/fig_response_get_overtime_list.png}}
...
\textit{Get User Overtime List} adalah \textit{endpoint} API yang berfungsi untuk mengambil semua permintaan lembur (\textit{Overtime}) milik pengguna yang sedang melakukan autentikasi. API ini digunakan di sisi \textit{frontend} untuk menampilkan daftar permintaan lembur milik pengguna dalam page \textit{My Overtime}.
Gambar \ref{fig:flowchart_get_user_overtime_list} merupakan \textit{flowchart} API \textit{Get User Overtime List}. \textit{Flowchart} dimulai dari pengecekan autentikasi pengguna. Jika autentikasi gagal, sistem akan mengembalikan \textit{error} 401. Jika autentikasi berhasil, sistem akan memeriksa \textit{permission overtime} untuk mengakses data permintaan lembur. Jika pengguna tidak ada \textit{permission overtime}, sistem akan mengembalikan \textit{error} 403. Setelah itu, sistem mengambil data semua permintaan lembur milik pengguna yang sedang melakukan autentikasi. Setelah data berhasil diambil, sistem mengembalikan respons (\textit{response}) kepada pengguna yang berisi daftar permintaan lembur milik pengguna.
\begin{figure}[H]
    \centering
...
Pada gambar \ref{fig:request_get_user_overtime_list} menunjukkan contoh \textit{request} untuk API \textit{Get User Overtime List} yang dikirimkan ke \textit{endpoint} \texttt{/overtime/user} menggunakan metode HTTP \textit{GET} dengan menyertakan beberapa \textit{query parameters} untuk kebutuhan \textit{pagination}, seperti \texttt{page} dan \texttt{row}.
\begin{figure}[H]
    \centering
...
Pada gambar \ref{fig:response_get_user_overtime_list} menunjukkan contoh \textit{response} untuk API \textit{Get User Overtime List} yang diterima dari \textit{server}, berisi daftar permintaan lembur milik pengguna yang sedang melakukan autentikasi, termasuk informasi seperti \textit{overtime ID}, tanggal lembur, durasi, dan status persetujuan.
\begin{figure}[H]
    \centering
...
\textit{Get Overtime Detail by ID} adalah \textit{endpoint} API yang berfungsi untuk mengambil detail permintaan lembur (\textit{Overtime}) berdasarkan \textit{ID} lembur yang diberikan. API ini digunakan di sisi \textit{frontend} untuk menampilkan informasi rinci mengenai permintaan lembur tertentu dalam page \textit{Overtime Detail}.
Gambar \ref{fig:flowchart_get_overtime_detail_by_id} merupakan \textit{flowchart} API \textit{Get Overtime Detail by ID}. \textit{Flowchart} dimulai dari pengecekan autentikasi pengguna. Jika autentikasi gagal, sistem akan mengembalikan \textit{error} 401. Jika autentikasi berhasil, sistem akan memeriksa \textit{permission user overtime} atau \textit{permission overtime management} untuk mengakses data permintaan lembur. Jika pengguna tidak ada \textit{permission user overtime} atau \textit{permission overtime management}, sistem akan mengembalikan \textit{error} 403. Setelah itu, sistem mengambil data detail permintaan lembur berdasarkan \textit{ID} lembur yang diberikan dari \textit{path parameters}. Jika data permintaan lembur tidak ditemukan, sistem akan mengembalikan \textit{error} 404. Setelah data berhasil diambil, sistem mengembalikan respons (\textit{response}) kepada pengguna yang berisi detail permintaan lembur yang diminta.
\begin{figure}[H]
    \centering
    \fbox{\includegraphics[height=0.6\textheight]{assets/pics/fig_flowchart_get_overtime_detail_by_id.jpg}}
    \caption{\textit{Flowchart} alur sistem API \textit{Get Overtime Detail by ID}}
    \label{fig:flowchart_get_overtime_detail_by_id}
\end{figure}

Pada gambar \ref{fig:request_get_overtime_detail_by_id} menunjukkan contoh \textit{request} untuk API \textit{Get Overtime Detail by ID} yang dikirimkan ke \textit{endpoint} \texttt{/overtime/:id} menggunakan metode HTTP \textit{GET} dengan menyertakan \textit{path parameters}, \texttt{id}, untuk menentukan permintaan lembur yang ingin diambil detailnya.
...
Pada gambar \ref{fig:response_get_overtime_detail_by_id} menunjukkan contoh \textit{response} untuk API \textit{Get Overtime Detail by ID} yang diterima dari \textit{server}, berisi informasi rinci mengenai permintaan lembur tertentu, termasuk rincian seperti nama pengguna, tanggal lembur, jam lembur, durasi, alasan lembur, status persetujuan, deskripsi lembur, dan lain-lain.
\begin{figure}[H]
    \centering
    \fbox{\includegraphics[width=0.65\textwidth]{assets/pics/fig_response_get_overtime_detail_by_id.png}}
...
\textit{Delete Overtime by ID} adalah \textit{endpoint} API yang berfungsi untuk menghapus permintaan lembur (\textit{Overtime}) berdasarkan \textit{ID} lembur yang diberikan. API ini digunakan di sisi \textit{frontend} untuk menghapus permintaan lembur tertentu dalam page \textit{Overtime Detail}.
Gambar \ref{fig:flowchart_delete_overtime_by_id} merupakan \textit{flowchart} API \textit{Delete Overtime by ID}. \textit{Flowchart} dimulai dari pengecekan autentikasi pengguna. Jika autentikasi gagal, sistem akan mengembalikan \textit{error} 401. Jika autentikasi berhasil, sistem akan memeriksa \textit{permission overtime management} untuk menghapus data permintaan lembur. Jika pengguna tidak ada \textit{permission overtime management}, sistem akan mengembalikan \textit{error} 403. Setelah itu, sistem menghapus data permintaan lembur berdasarkan \textit{ID} lembur yang diberikan dari \textit{path parameters}. Jika data permintaan lembur tidak ditemukan, sistem akan mengembalikan \textit{error} 404. Setelah data permintaan lembur berhasil dihapus, sistem mengembalikan respons (\textit{response}) kepada pengguna yang berisi informasi mengenai keberhasilan penghapusan permintaan lembur.
\begin{figure}[H]
    \centering
...
Pada gambar \ref{fig:request_delete_overtime_by_id} menunjukkan contoh \textit{request} untuk API \textit{Delete Overtime by ID} yang dikirimkan ke \textit{endpoint} \texttt{/overtime/:id} menggunakan metode HTTP \textit{DELETE} dengan menyertakan \textit{path parameters}, \texttt{id}, untuk menentukan permintaan lembur yang ingin dihapus.

\begin{figure}[H]
    \centering
    \fbox{\includegraphics[width=1\textwidth]{assets/pics/fig_request_delete_overtime_by_id.png}}
    \caption{Contoh \textit{request} API \textit{Delete Overtime by ID}}
    \label{fig:request_delete_overtime_by_id}
\end{figure}

Pada gambar \ref{fig:response_delete_overtime_by_id} menunjukkan contoh \textit{response} untuk API \textit{Delete Overtime by ID} yang diterima dari \textit{server}, berisi \textit{status code} 200 dengan pesan sukses mengindikasikan bahwa permintaan lembur telah berhasil dihapus.
\begin{figure}[H]
    \centering
    \fbox{\includegraphics[height=0.2\textheight]{assets/pics/fig_response_delete_overtime_by_id.png}}
